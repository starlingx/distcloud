...
EXPECTED_FIELDS = [
    "id", "peer_group_name", "group_priority", "group_state",
...
class BaseTestSubcloudPeerGroupController(
    DCManagerApiTest, SubcloudPeerGroupAPIMixin
):
...
class TestSubcloudPeerGroupControllerPost(
    ...
    def test_post_fails_with_invalid_system_leader_name(self):
        ...
        invalid_values = [
            '123', 'a' * (subcloud_peer_group.MAX_SUBCLOUD_PEER_GROUP_NAME_LEN + 1)
        ]
    ...
    def test_post_fails_with_invalid_group_priority(self):
        ...
        for index, invalid_value in enumerate(invalid_values, start=1):
            ...
            self._assert_pecan_and_response(
                response, http.client.BAD_REQUEST, 'Invalid group-priority',
                call_count=index
            )
    ...
    def test_post_fails_with_invalid_max_subcloud_rehoming(self):
        ...
        for index, invalid_value in enumerate(invalid_values, start=1):
            ...
            self._assert_pecan_and_response(
                response, http.client.BAD_REQUEST, 'Invalid max-subcloud-rehoming',
                call_count=index
            )
...
class TestSubcloudPeerGroupControllerGet(BaseTestSubcloudPeerGroupController):
    ...
    def test_get_succeeds_for_subcloud_in_multiple_deploy_states(self):
        ...
        deploy_states = [
            ...
            consts.DEPLOY_STATE_REHOME_PREP_FAILED, consts.DEPLOY_STATE_SECONDARY,
            consts.DEPLOY_STATE_DONE, consts.DEPLOY_STATE_ABORTING_CONFIG
        ]
        ...
        for deploy_state in deploy_states:
            ...
            db_api.subcloud_update(
                self.ctx, subcloud.id, name=f'subcloud-{deploy_state}',
                management_state='managed', peer_group_id=self.peer_group.id,
...
class TestSubcloudPeerGroupControllerPatch(BaseTestSubcloudPeerGroupControllerPatch):
    ...
    def setUp(self):
        ...
        self.params = {
            'peer-group-name': 'fake value', 'group-priority': 1,
            'group-state': 'enabled', 'max-subcloud-rehoming': 1,
            'system-leader-id': FAKE_UUID, 'system-leader-name': 'fake value',
    ...
    def test_patch_fails_when_unable_to_sync(self):
        ...
        self._assert_pecan_and_response(
            response, http.client.FORBIDDEN,
    ...
    def test_patch_fails_with_system_leader_name_length_greater_than_maximum(self):
        ...
        self.params['system-leader-name'] = \
            'a' * (subcloud_peer_group.MAX_SUBCLOUD_PEER_GROUP_NAME_LEN + 1)
...
class TestSubcloudPeerGroupControllerPatchMigrate(
    ...
    def test_patch_migrate_fails_with_rpc_client_remote_error(self):
        ...
        self.mock_rpc_client().batch_migrate_subcloud.side_effect = \
...
class TestSubcloudPeerGroupControllerPatchAudit(
    ...
    def setUp(self):
        ...
        self.params = {
            'peer_uuid': FAKE_UUID, 'peer_group_name': 'fake name',
            'group_priority': 1, 'group_state': 'deployed', 'system_leader_id': 1,
            'system_leader_name': 'fake name', 'migration_status': 'fake status'
        }
    ...
    def test_patch_audit_fails_without_peer_group_name(self):
        ...
        self._assert_pecan_and_response(
            response, http.client.BAD_REQUEST, 'Unable to audit peer group '
            f'{self.peer_group.peer_group_name}, missing peer_group_name'
        )
    ...
    def test_patch_audit_fails_without_group_priority(self):
        ...
        self._assert_pecan_and_response(
            response, http.client.BAD_REQUEST, 'Unable to audit peer group '
            f'{self.peer_group.peer_group_name}, missing group_priority'
        )
    ...
    def test_patch_audit_fails_without_group_state(self):
        ...
        self._assert_pecan_and_response(
            response, http.client.BAD_REQUEST, 'Unable to audit peer group '
            f'{self.peer_group.peer_group_name}, missing group_state'
        )
    ...
    def test_patch_audit_fails_without_system_leader_id(self):
        ...
        self._assert_pecan_and_response(
            response, http.client.BAD_REQUEST, 'Unable to audit peer group '
            f'{self.peer_group.peer_group_name}, missing system_leader_id'
        )
    ...
    def test_patch_audit_fails_without_system_leader_name(self):
        ...
        self._assert_pecan_and_response(
            response, http.client.BAD_REQUEST, 'Unable to audit peer group '
            f'{self.peer_group.peer_group_name}, missing system_leader_name'
        )
    ...
    def test_patch_audit_fails_without_migration_status(self):
        ...
        self._assert_pecan_and_response(
            response, http.client.BAD_REQUEST, 'Unable to audit peer group '
            f'{self.peer_group.peer_group_name}, missing migration_status'
        )
...
class TestSubcloudPeerGroupControllerDelete(BaseTestSubcloudPeerGroupController):
    ...
    def test_delete_fails_when_association_exists(self):
        ...
        system_peer = db_api.system_peer_create(
            self.ctx, str(uuid.uuid4()), 'SystemPeer1', 'http://127.0.0.1:5000',
            'admin', 'password', '128.128.128.1'
        )
        db_api.peer_group_association_create(
            self.ctx, self.subcloud.peer_group_id, system_peer.id,
            self.peer_group.group_priority, consts.ASSOCIATION_TYPE_PRIMARY,
