...
FAKE_ASSOCIATION_PEER_GROUP_ID = FAKE_SITE0_PEER_GROUP_ID
FAKE_ASSOCIATION_SYSTEM_PEER_ID = FAKE_SYSTEM_PEER_ID
...
class TestSystemPeerManager(base.DCManagerTestCase):
    ...
    def setUp(self):
        ...
        self.peer = self.create_system_peer_static(self.ctx, peer_name="SystemPeer1")
        ...
        self.association = self.create_peer_group_association_static(
            self.ctx, system_peer_id=self.peer.id, peer_group_id=self.peer_group.id
        )
    ...
    @staticmethod
    def create_subcloud_with_pg_static(ctxt, peer_group_id, rehome_data=None, **kwargs):
        subcloud = fake_subcloud.create_fake_subcloud(ctxt, **kwargs)
        return db_api.subcloud_update(
            ctxt, subcloud.id, peer_group_id=peer_group_id, rehome_data=rehome_data
        )
    ...
    def create_subcloud_peer_group_static(ctxt, **kwargs):
        values = {
            ...
            "max_subcloud_rehoming": FAKE_SITE0_PEER_GROUP_MAX_SUBCLOUDS_REHOMING,
    ...
    def test_sync_subclouds(self):
        ...
        self.mock_dc_client().update_subcloud.assert_has_calls(
            [
                ...
                mock.call(
                    FAKE_SITE1_SUBCLOUD1_REGION_NAME,
                    files=None,
                    data={"peer_group": str(FAKE_SITE1_PEER_GROUP_ID)},
                    is_region_name=True,
                ),
                mock.call(
                    FAKE_SITE1_SUBCLOUD2_REGION_NAME,
                    files=None,
                    data={"peer_group": str(FAKE_SITE1_PEER_GROUP_ID)},
                    is_region_name=True,
                ),
        ...
        self.mock_dc_client().add_subcloud_with_secondary_status.assert_called_once()
    ...
    def test_delete_peer_group_association(self):
        ...
        self.mock_dc_client().delete_subcloud.assert_has_calls(
            [mock.call(subcloud1.name), mock.call(subcloud2.name)]
        )
    ...
    def test_delete_peer_group_association_peer_site_association_not_exist(self):
        ...
        self.peer_group_association.side_effect = [
            dccommon_exceptions.PeerGroupAssociationNotFound
        ]
    ...
    @mock.patch.object(system_peer_manager.SystemPeerManager, "update_sync_status")
    def test_update_association_sync_status(self, mock_update_sync_status):
        ...
        self.spm.update_association_sync_status(
            self.ctx, self.peer_group.id, consts.ASSOCIATION_SYNC_STATUS_OUT_OF_SYNC
        )
    ...
    def test_update_association_sync_status_when_peer_site_down(
        ...
        self.assertEqual(consts.ASSOCIATION_SYNC_STATUS_FAILED, association.sync_status)
        self.assertEqual(
            "Failed to update sync_status, because the peer site is unreachable.",
    ...
    def test_update_subcloud_peer_group(self):
        ...
        peer_group_kwargs = {
            ...
            "max-subcloud-rehoming": FAKE_SITE1_PEER_GROUP_MAX_SUBCLOUDS_REHOMING,
        }
    ...
    def test_update_sync_status_exception(self, mock_client):
        ...
        self.assertEqual(
            consts.ASSOCIATION_SYNC_STATUS_FAILED, association_new.sync_status
        )
    ...
    def test_handle_association_operations_in_progress(self):
        ...
        db_api.peer_group_association_update(
            self.ctx, self.peer.id, sync_status=consts.ASSOCIATION_SYNC_STATUS_SYNCING
        )
        self.spm.handle_association_operations_in_progress()
        associations = db_api.peer_group_association_get(self.ctx, self.association.id)
        self.assertEqual(
            consts.ASSOCIATION_SYNC_STATUS_FAILED, associations.sync_status
        )
        Calls = [
            mock.call("Identifying associations in transitory stages."),
            mock.call("Changing association 1 sync status from syncing to failed"),
        ]
    ...
    def test_valid_for_subcloud_sync_deploy_status_secondary(self):
        ...
        subcloud1 = self._create_subcloud(
            "subcloud1",
            json.dumps(fake_subcloud.FAKE_SUBCLOUD_INSTALL_VALUES),
    ...
    def test_is_valid_for_subcloud_sync_missing_rehome_data(self):
        # Create local dc subcloud1 mock data in database
        subcloud1 = self._create_subcloud("subcloud1", data_install=self.data_install)
        ret_msg = self.spm._is_valid_for_subcloud_sync(subcloud1)
        msg = "Subcloud subcloud1 (region_name: subcloud1) does not have rehome_data."
    ...
    def test_is_valid_for_subcloud_sync_missing_saved_payload_empty(self):
        ...
        ret_msg = self.spm._is_valid_for_subcloud_sync(subcloud1)
        msg = (
            f"Subcloud {subcloud1.name} (region_name: {subcloud1.region_name}) "
            "saved_payload is empty."
        )
        self.assertEqual(msg, ret_msg)
    ...
    def test_is_valid_for_subcloud_sync_missing_bootstrap_address(self):
        # Create local dc subcloud1 mock data in database
        rehome_data = {
            "saved_payload": {"systemcontroller_gateway_address": "192.168.204.101"}
        }
        # Create local dc subcloud1 mock data in database
        subcloud1 = self._create_subcloud(
            "subcloud1",
    ...
    def test_is_valid_for_subcloud_sync_missing_saved_payload(self):
        ...
        ret_msg = self.spm._is_valid_for_subcloud_sync(subcloud1)
        msg = (
            f"Subcloud {subcloud1.name} (region_name: subcloud1) "
    ...
    def test_valid_for_subcloud_sync_missing_systemcontroller_gateway_addr(self):
        # Create local dc subcloud1 mock data in database
        rehome_data = {"saved_payload": {"bootstrap-address": "192.168.10.10"}}
        # Create local dc subcloud1 mock data in database
        subcloud1 = self._create_subcloud(
            "subcloud1",
            rehome_data=json.dumps(rehome_data),
    ...
    def test_sync_subclouds_delete_subcloud_exception(self):
        ...
        self.mock_log.error.assert_called_once_with("Subcloud delete failed: boom")
    ...
    def test_sync_subcloud_peer_group_fail(self):
        ...
        peer = self.create_system_peer_static(
            self.ctx, peer_uuid=111, peer_name="SystemPeer"
        )
        peer_group = self.create_subcloud_peer_group_static(
            self.ctx, peer_group_name="SubcloudPeerGroup3"
        )
        association = self.create_peer_group_association_static(
            self.ctx, system_peer_id=peer.id, peer_group_id=peer_group.id
        )
        self.assertRaises(
            exceptions.PeerGroupAssociationTargetNotMatch,
    ...
    def test_sync_subcloud_peer_group_failed_sp_notfound(self):
        ...
        self.mock_log.error.assert_called_once_with(
            f"Peer Site System Peer {self.mock_get_local_system().uuid} does not exist."
        )
