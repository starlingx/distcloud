...
def subcloud_audits_get_all_need_audit(context, last_audit_threshold):
    with read_session() as session:
        result = (
            ...
            .join(
                models.Subcloud, models.Subcloud.id == models.SubcloudAudits.subcloud_id
            )
            .filter_by(deleted=0)
            .filter(
...
def subcloud_get_with_status(context, subcloud_id):
    result = (
        model_query(context, models.Subcloud, models.SubcloudStatus)
        .outerjoin(
...
def subcloud_status_get(context, subcloud_id, endpoint_type):
    result = (
        model_query(context, models.SubcloudStatus)
        .filter_by(deleted=0)
        .filter_by(subcloud_id=subcloud_id)
...
def subcloud_status_get_all(context, subcloud_id):
    return (
        model_query(context, models.SubcloudStatus)
        .filter_by(deleted=0)
        .join(models.Subcloud, models.SubcloudStatus.subcloud_id == models.Subcloud.id)
        .filter(models.Subcloud.id == subcloud_id)
        .all()
    )
...
def subcloud_status_delete(context, subcloud_id, endpoint_type):
    with write_session() as session:
        subcloud_status_ref = subcloud_status_get(context, subcloud_id, endpoint_type)
...
def sw_update_strategy_update(
    ...
    with write_session() as session:
        sw_update_strategy_ref = sw_update_strategy_get(
...
def sw_update_opts_get(context, subcloud_id):
    result = (
        model_query(context, models.SwUpdateOpts)
        .filter_by(deleted=0)
        .filter_by(subcloud_id=subcloud_id)
        .first()
    )
...
def sw_update_opts_get_all_plus_subcloud_info(context):
    result = (
        model_query(context, models.Subcloud, models.SwUpdateOpts)
        .outerjoin(
            models.SwUpdateOpts,
...
def sw_update_opts_default_get(context):
    result = (
        model_query(context, models.SwUpdateOptsDefault).filter_by(deleted=0).first()
    )
...
def system_peer_get(context, peer_id):
    try:
        result = (
            model_query(context, models.SystemPeer)
            .filter_by(deleted=0)
...
def peer_group_get_for_system_peer(context, peer_id):
    return (
        model_query(context, models.SubcloudPeerGroup)
        .join(
            models.PeerGroupAssociation,
            models.SubcloudPeerGroup.id == models.PeerGroupAssociation.peer_group_id,
        )
...
def subcloud_group_get(context, group_id):
    try:
        result = (
            model_query(context, models.SubcloudGroup)
            .filter_by(deleted=0)
...
def peer_group_association_get(context, association_id):
    try:
        ...
        raise exception.InvalidParameterValue(
            err="Multiple entries found for peer group association %s" % association_id
        )
...
def strategy_step_get_by_name(context, name):
    result = (
        model_query(context, models.StrategyStep)
        .filter_by(deleted=0)
        .join(models.Subcloud, models.StrategyStep.subcloud_id == models.Subcloud.id)
        .filter(models.Subcloud.name == name)
        .first()
    )
