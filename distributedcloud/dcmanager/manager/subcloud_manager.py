...
class SubcloudManager(manager.Manager):
    ...
    def _setup_auto_restore_without_install(
        ...
        seed_iso_path = self._generate_auto_restore_seed_iso(
            subcloud, restore_ctx.overrides_file, payload
        )
    ...
    def _restore_subcloud_backup(
        self, context, payload: dict, subcloud: Subcloud
    ) -> tuple[Subcloud, bool]:
        ...
        try:
            ...
            restore_ctx = self._prepare_restore_files(
                payload, subcloud, restore_mode, bootstrap_address
            )
    ...
    def _restore_subcloud_backup_on_site(
        ...
        if not payload.get("local_only"):
            backup_source_path = utils.find_central_subcloud_backup(
                subcloud.name, payload.get("release", SW_VERSION)
            )
            LOG.debug(
