...
class Token(object):
    ...
    def _get_service_url(self, service_type, service_name, interface_type, region_name):
        ...
        for catalog in self.data["token"]["catalog"]:
            if (
                catalog["type"] == service_type
                and catalog["name"] == service_name
                and len(catalog["endpoints"]) != 0
            ):
                for endpoint in catalog["endpoints"]:
                    if (
                        endpoint["interface"] == interface_type
                        and endpoint["region"] == region_name
                    ):
                        return endpoint["url"]
        return None

    def get_service_admin_url(self, service_type, service_name, region_name=None):
        """Search the catalog of a service for the administrative url

        Returns: admin url or None on failure
        """
        return self._get_service_url(service_type, service_name, "admin", region_name)

    def get_service_internal_url(self, service_type, service_name, region_name=None):
        """Search the catalog of a service for the internal url

        Returns: internal url or None on failure
        """
        return self._get_service_url(
            service_type, service_name, "internal", region_name
        )

    def get_service_public_url(self, service_type, service_name, region_name=None):
        """Search the catalog of a service for the public url

        Returns: public url or None on failure
        """
        return self._get_service_url(service_type, service_name, "public", region_name)
    ...
    def get_service_url(self, service_type, service_name, region_name=None):
        ...
        Returns: admin url or None on failure
        """
        return self.get_service_admin_url(service_type, service_name, region_name)

    def get_full_str(self):
        """Formats the entire token, used only for debugging."""
        return "id: {}, expired: {}, region_name: {}, expires_at: {}, data: {}".format(
            self.token_id,
            self.expired,
            self.region_name,
